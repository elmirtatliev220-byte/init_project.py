...
async def toggle_protection(data: ProtectionRequest, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    await check_access(db, data.chat_id, user)
    
    # Используем raw SQL для надежности (если модель еще не обновлена)
    # 1. Получаем текущий статус
    stmt_get = text("SELECT protection_enabled FROM channels WHERE tg_id = :cid")
    res = await db.execute(stmt_get, {"cid": data.chat_id})
    current_status = res.scalar() or 0
    
    # 2. Меняем на противоположный
    new_status = 0 if current_status else 1
    
    stmt_update = text("UPDATE channels SET protection_enabled = :s WHERE tg_id = :cid")
    await db.execute(stmt_update, {"s": new_status, "cid": data.chat_id})
    await db.commit()
    
    return {
        "status": "success", 
        "is_enabled": bool(new_status),
        "message": "🛡 Защита активирована" if new_status else "🛡 Защита выключена"
    }
