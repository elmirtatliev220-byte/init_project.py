...
async def check_access(session, channel_id: int, user_id: int):
    """
    Проверяет, принадлежит ли канал пользователю.
    Принимает user_id (int) вместо объекта User для надежности.
    """
    # Используем select(1) для оптимизации
    stmt = select(1).select_from(Channel).where(Channel.tg_id == channel_id, Channel.owner_id == user_id).limit(1)
    result = await session.execute(stmt)
...
async def save_wallet(data: WalletRequest, user = Depends(get_current_user)):
    async with async_session() as session:
...
async def resolve_channel(data: ChannelCheckRequest, user = Depends(get_current_user)):
    ...
    try:
        ...
        photo_url = None
        if chat.photo:
            # Для превью возвращаем file_id или ссылку, если есть (здесь упрощенно)
            # В реальном проекте лучше скачать, но для превью можно вернуть просто инфо
            pass
...
async def add_channel(data: ChannelAddRequest, user: User = Depends(get_current_user)):
    """Добавляет канал в систему после проверки через /resolve_channel"""
    try:
        # ID канала должен быть числом, т.к. он прошел валидацию
        channel_tg_id = int(data.tg_id)

        # 1. Получаем актуальную информацию о канале
        try:
            chat = await bot.get_chat(channel_tg_id)
        except Exception:
            return {"status": "error", "message": f"Не удалось найти канал с ID {channel_tg_id}."}

        # 2. Повторная проверка прав (безопасность)
        try:
        ...
        async with async_session() as session:
            # 3. Проверяем, не добавлен ли канал уже
            ...
            # 5. Добавляем в базу
            new_channel = Channel(tg_id=chat.id, title=chat.title, owner_id=user.id, photo_url=photo_path)
...
async def ai_generate_image(data: AIRequest, user = Depends(get_current_user)):
    """
