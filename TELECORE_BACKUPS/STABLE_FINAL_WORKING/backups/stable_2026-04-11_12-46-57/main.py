...
if settings.GEMINI_API_KEY:
    ...
    logger.warning("⚠️ Gemini AI не настроен.")
            ...
            async with async_session() as session:
                ...
                for post in posts:
                    try:
                        ...
                        logger.info(f"✅ Пост {post.id} отправлен в {post.channel_id}")
                    except TelegramRetryAfter as e:
                        print(f"⏳ Лимит Telegram. Ждем {e.retry_after} сек...")
                        logger.warning(f"⏳ Flood limit. Waiting {e.retry_after}s")
...
async def lifespan(app: FastAPI):
    ...
    print("✅ База готова.")
    logger.info("✅ База готова. Запуск ботов...")
...
# --- STATIC ROUTES ---
@app.get("/")
async def read_index(): return FileResponse("index.html") if os.path.exists("index.html") else {"error": "404"}

@app.get("/api/config")
async def get_config(): return {"api_url": os.getenv("NGROK_URL") or os.getenv("API_URL")}
...
async def get_user_channels(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    return [{"id": c.tg_id, "title": c.title, "photo": c.photo_url, "owner_id": user.tg_id} for c in channels]
...
class LocalChannelAddRequest(BaseModel):
    link: Optional[str] = None
    tg_id: Optional[Union[str, int]] = None
    id: Optional[Union[str, int]] = None

@app.post("/api/add_channel")
async def add_channel(data: LocalChannelAddRequest, user: User = Depends(get_current_user)):
    ...
    try:
        ...
        print(f"❌ КРИТИЧЕСКАЯ ОШИБКА /api/add_channel: {e}")
        logger.error(f"❌ КРИТИЧЕСКАЯ ОШИБКА /api/add_channel: {e}")
...
async def schedule_post(
    ...
    try:
        ...
        async with async_session() as session:
            ...
            logger.info(f"📅 Пост запланирован на {p_time} для {channel_id}")
...
async def update_channel_stats(channel_id: int):
    ...
    try:
        ...
        logger.error(f"⚠️ Ошибка обновления подписчиков для {channel_id}: {e}")
        ...
        for post in posts:
            try:
                ...
                if "message to forward not found" in str(e) or "MESSAGE_ID_INVALID" in str(e):
                    ...
                    logger.info(f"🗑 Пост {post.message_id} удален в Telegram.")
                    await session.delete(post)
                else:
                    print(f"⚠️ Не удалось обновить метрики сообщения {post.message_id}: {e}")
                    logger.warning(f"⚠️ Ошибка метрик {post.message_id}: {e}")
            except Exception as e:
                print(f"⚠️ Не удалось обновить метрики сообщения {post.message_id}: {e}")
                logger.error(f"⚠️ Критическая ошибка метрик: {e}")
