...
def create_backup() -> None:
    ...
    files_to_save = [
        ...
        "app/bot/main_bot.py", "full_audit_cleanup.py"
    ]
...
def audit() -> None:
    ...
    # 2. Обновление ссылки Mini App в боте (main_bot.py)
    ...
    if os.path.exists(bot_file):
        try:
            ...
            # Регулярное выражение ищет строку web_app_url и заменяет URL, сохраняя генерацию таймстемпа
            pattern = r'web_app_url = f"https?://.*?\?v=\{int\(time\.time\(\)\)\}"'
            replacement = f'web_app_url = f"{GITHUB_PAGE_URL}?v={{int(time.time())}}"'
            new_bot_content = re.sub(pattern, replacement, bot_content)
            
            with open(bot_file, "w", encoding="utf-8") as f:
                f.write(new_bot_content)
            print(f"✅ Ссылка на Mini App обновлена в {bot_file}")
        except Exception as e:
            print(f"⚠️ Ошибка при обновлении {bot_file}: {e}")
