...
let pendingChannelId = null; // BUGFIX: Глобально объявляем переменную для хранения ID проверенного канала
...
function toggleAddChannel(show) {
    ...
    if (show) {
        ...
        tg.BackButton.show();
        pendingChannelId = null; // BUGFIX: Сбрасываем ID при каждом открытии окна, чтобы избежать добавления старого канала
    } else {
...
async function checkChannel() {
    ...
    try {
        ...
        if (data.status === 'success') {
            ...
            // BUGFIX: Сохраняем полученный от бэкенда стабильный ID канала
            pendingChannelId = data.id; 
        } else {
            tg.showAlert(data.message);
            document.getElementById('channel-preview').classList.add('hidden');
            pendingChannelId = null; // BUGFIX: Сбрасываем ID в случае ошибки проверки
        }
    } catch (e) {
...
function confirmAddChannel() {
    // BUGFIX: Используем сохраненный ID канала после проверки, а не сырую ссылку из инпута
    if (pendingChannelId) {
        addChannelToServer(pendingChannelId); // Передаем на сервер уже проверенный ID
        toggleAddChannel(false);
    } else {
        tg.showAlert("Сначала проверьте ссылку на канал с помощью кнопки 'Проверить'.");
    }
}
...
async function addChannelToServer(channelTarget) {
    tg.MainButton.showProgress();
    try {
        const res = await apiFetch(`/api/add_channel`, {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({ tg_id: channelTarget }) // BUGFIX: Отправляем на сервер ID, а не ссылку
        });
        const result = await res.json();
        if(result.status === 'success') {
            // BUGFIX: Используем более информативное сообщение от бэкенда
