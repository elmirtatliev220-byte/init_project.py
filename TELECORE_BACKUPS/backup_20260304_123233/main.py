...
import json
import logging
import time # Для замера времени ответа
...
# FIX: Определяем абсолютный путь к корню проекта, чтобы избежать ошибок с относительными путями.
# Это решает ошибку "File not found" при запуске из подпапок.
PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))

# Создаем папку для загрузок, если нет
os.makedirs("static/uploads", exist_ok=True)

# --- НАСТРОЙКА ЛОГИРОВАНИЯ (v2026) ---
logging.basicConfig(
    level=logging.INFO,
...
if settings.GEMINI_API_KEY:
    try:
        ...
        logger.info("🤖 Gemini AI клиент инициализирован (v1).")
...
async def add_reactions(chat_id: int, message_id: int):
    ...
    try:
        ...
        logger.warning(f"⚠️ Не удалось поставить реакции: {e}")
        ...
        try:
            ...
            if datetime.now(timezone.utc) - last_metric_update > timedelta(hours=1):
                ...
                try:
                    ...
                    logger.info("✅ Статистика (охваты и подписчики) успешно обновлена.")
                except Exception as e:
                    logger.error(f"⚠️ Ошибка фонового обновления метрик: {e}")
            ...
            async with async_session() as session:
                ...
                for post in posts:
                    try:
                        ...
                        post.is_sent = 1
                        logger.info(f"✅ Пост {post.id} успешно отправлен в канал {post.channel_id}")
            ...
            logger.critical(f"🚨 Ошибка в цикле планировщика: {e}")
...
async def read_root(request: Request):
    """Отдает index.html и динамически вставляет в него URL из конфига."""
    index_path = os.path.join(PROJECT_ROOT, "index.html")
    with open(index_path, "r", encoding="utf-8") as f:
        html_content = f.read()

    # Заменяем старый, жестко прописанный URL на актуальный из .env
    # Это более надежно, чем заменять URL в script.js, так как URL определяется в index.html
    placeholder_to_replace = "const API_URL = '    https://felicidad-selfsame-veritably.ngrok-free.dev';"
    # HOTFIX: Используем безопасный getattr для совместимости со старым конфигом
    ngrok_url = getattr(settings, 'NGROK_URL', os.getenv('NGROK_URL'))
    actual_url_line = f"const API_URL = '{ngrok_url}';"
    modified_html = html_content.replace(placeholder_to_replace, actual_url_line)
...
async def get_script():
    """
    Отдает script.js, динамически заменяя в нем URL-заглушку
    на актуальный NGROK_URL из настроек.
    """
    ...
    try:
        ...
        # Строка для поиска (из ТЗ, может быть let или const)
        placeholder_url = "https://felicidad-selfsame-veritably.ngrok-free.dev"
        
        # Безопасное получение URL из настроек или напрямую из окружения
        current_ngrok = getattr(settings, 'NGROK_URL', os.getenv('NGROK_URL'))
        modified_content = content.replace(placeholder_url, current_ngrok)
        
        return Response(content=modified_content, media_type="application/javascript")
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail=f"script.js not found at {script_path}")
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*", "https://elmirtatliev220-byte.github.io"],
...
async def add_channel(data: ChannelAddRequest, user: User = Depends(get_current_user)):
    try:
        logger.info(f"📥 Запрос на добавление канала: {data.tg_id}")
        ...
        if chat.photo:
            try:
                ...
                logger.warning(f"⚠️ Не удалось скачать лого канала: {e}")
...
async def delete_channel(request: ChannelDeleteRequest, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    try:
        ...
        # Т.к. в models.py нет ForeignKey с cascade, удаляем вручную:
...
async def delete_scheduled(data: DeleteRequest, user: User = Depends(get_current_user)):
    try:
        async with async_session() as session:
            ...
            await session.commit()
            logger.info(f"🗑 Удален запланированный пост ID: {data.id}")
        return {"status": "success"}
    except Exception as e:
        return {"status": "error", "message": str(e)}
...
async def schedule_post(
    ...
    try:
        ...
        async with async_session() as session:
            ...
            now = datetime.now(timezone.utc)
            logger.info(f"📅 Пост запланирован на {p_time} (через {p_time - now}) для канала {channel_id}")
...
async def send_post(
    ...
    try:
        ...
        if poll_question:
            ...
            msg = await bot.send_message(chat_id=channel_id, text=text or "", parse_mode="HTML")
        ...
        if msg:
            ...
            async with async_session() as session:
                ...
                session.add(new_p_stat)
                await session.commit()
            logger.info(f"🚀 Пост отправлен мгновенно в канал {channel_id}")
        return {"status": "success"}
    except Exception as e:
        return {"status": "error", "message": str(e)}
...
async def update_channel_stats(channel_id: int):
    ...
    try:
        ...
        logger.warning(f"⚠️ Ошибка обновления подписчиков для {channel_id}: {e}")
    ...
    async with async_session() as session:
        ...
        for post in posts:
            try:
                ...
                err_str = str(e)
                if "message to forward not found" in err_str or "MESSAGE_ID_INVALID" in err_str:
                    logger.info(f"🗑 Пост {post.message_id} удален в Telegram. Удаляем из статистики.")
                    await session.delete(post)
                elif "message can't be forwarded" in err_str:
                    # Сообщение защищено (запрет пересылки). Удаляем из статистики, чтобы остановить попытки.
                    await session.delete(post)
                else:
                    logger.warning(f"⚠️ Не удалось обновить метрики сообщения {post.message_id}: {e}")
            except Exception as e:
                logger.warning(f"⚠️ Не удалось обновить метрики сообщения {post.message_id}: {e}")
...
async def ai_process(data: AIRequest, user: User = Depends(get_current_user)):
    ...
    try:
        system_instruction = (
            "Ты — профессиональный SMM-редактор. Твоя задача — упаковать информацию в структурированный пост.\n\n"
            "ПРАВИЛА ОФОРМЛЕНИЯ:\n\n"
            "Заголовок: Первая строка — короткая, жирным шрифтом через <b>ЗАГОЛОВОК</b>. В начале и конце заголовка по ОДНОМУ эмодзи (например, 🔥 или 🚀).\n\n"
            "Лид-абзац: Одно предложение, вводящее в курс дела. Никаких «Привет, друзья» и «Давайте разберемся».\n\n"
            "Пустые строки: После заголовка, после лида и МЕЖДУ каждым пунктом списка — СТРОГО пустая строка (двойной перенос).\n\n"
            "Структура списка: Каждый пункт начинается с буллита (•). Подзаголовок пункта выдели жирным через <b>Название:</b>. Далее — суть одним коротким предложением.\n\n"
            "Эмодзи в списке: Только ОДИН тематический эмодзи в конце каждой строки списка.\n\n"
            "Финал: Краткий призыв к обсуждению и блок хештегов (не более 5-6 штук).\n\n"
            "Запреты: Запрещено использовать слова «сегодня», «на сегодняшний день», «на арене», «непростой диалог». Только факты.\n\n"
            "ВЫДАЙ ТОЛЬКО ЧИСТЫЙ HTML."
        )
        user_prompt = ""
        if data.action == "generate":
            user_prompt = f"Напиши увлекательный пост для Telegram-канала на тему: '{data.prompt}'. Используй эмодзи, делай абзацы. Стиль: живой, вовлекающий."
        elif data.action == "rewrite":
            user_prompt = f"Перепиши этот текст для Telegram, сделав его более кликабельным, интересным и легким для чтения. Сохрани смысл:\n\n{data.prompt}"
        elif data.action == "headlines":
            user_prompt = f"Придумай 5 кликбейтных и цепляющих заголовков для этого текста (выведи только список):\n\n{data.prompt}"
        elif data.action == "clickbait":
            user_prompt = f"Перепиши этот текст, сделав его максимально интригующим и виральным (кликбейтным). Добавь 'крючок' в начале. Текст:\n\n{data.prompt}"
        elif data.action == "shorten":
            user_prompt = f"Сократи этот текст для Telegram, оставив только самую суть. Убери воду, сделай списки, если уместно. Текст:\n\n{data.prompt}"
        elif data.action == "emoji":
            user_prompt = f"Добавь подходящие эмодзи в этот текст, чтобы он выглядел визуально привлекательно и живо, но не переборщи. Текст:\n\n{data.prompt}"
        elif data.action == "analyze":
            user_prompt = f"Проанализируй этот пост для Telegram. Оцени его виральный потенциал от 0 до 100. Дай 3 конкретных совета, как его улучшить. Текст:\n\n{data.prompt}"
        else:
        ...
        result_text = await loop.run_in_executor(None, lambda: process_ai_request_sync(full_prompt))
        ...
        if data.action != "analyze" and "Ошибка" not in result_text:
            ...
            # Простая защита: если количество <b> не равно </b>, закрываем тег в конце
