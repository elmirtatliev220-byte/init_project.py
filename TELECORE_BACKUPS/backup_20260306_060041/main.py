...
async def debug_requests(request: Request, call_next):
    ...
    logger.info(f"📡 СЕРВЕР ВИДИТ ЗАПРОС: {request.method} {request.url.path} от {request.client.host}")
...
async def get_config():
    """Возвращает публичный URL API для фронтенда"""
    return {"api_url": os.getenv("NGROK_URL") or os.getenv("API_URL")}
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "https://elmirtatliev220-byte.github.io", # Твой фронтенд на GitHub
        "https://felicidad-selfsame-veritably.ngrok-free.dev", # Твой ngrok
        "http://localhost:8080",
    ],
...
async def ai_process(data: AIRequest, user: User = Depends(get_current_user)):
    ...
    try:
        system_instruction = (
            "Ты — главный редактор Forbes. Пиши посты для Telegram. "
            "СТРУКТУРА: 1. Жирный заголовок <b>Текст</b>. 2. Суть в 3 абзацах. 3. Список через ✅. "
            "ФОРМАТ: Используй ТОЛЬКО HTML (<b>, <i>, <code>). "
            "ЗАПРЕТ: Не пиши 'Вот ваш пост', выдавай сразу готовый контент."
        )
        user_task = ""
        if data.action == "generate":
            user_task = f"Напиши пост на тему: '{data.prompt}'"
        elif data.action == "rewrite":
            user_task = f"Перепиши этот текст, сохранив смысл, но сделав его лучше: '{data.prompt}'"
        elif data.action == "shorten":
            user_task = f"Сократи этот текст, оставив только самую суть: '{data.prompt}'"
        elif data.action == "clickbait":
             user_task = f"Сделай этот текст максимально интригующим и виральным: '{data.prompt}'"
        elif data.action == "headlines":
             user_task = f"Придумай 5 ярких заголовков для этого текста: '{data.prompt}'"
        else: # Действие по умолчанию
            user_task = f"Обработай следующий текст/тему: '{data.prompt}'"
