...
async def cmd_start(message: types.Message):
    ...
    # ВАЖНО: Возвращаем путь к репозиторию. Если имя репозитория другое, замените 'init_project.py' на него.
    ...
    if not is_admin:
        ...
        if text:
            try:
                ...
                print(f"⏳ Flood control в модераторе (spam). Ждем {e.retry_after} сек.")
                await asyncio.sleep(e.retry_after)
            except Exception as e:
                print(f"⚠️ Ошибка модерации спама: {e}")
            ...
            async with async_session() as session:
                ...
                for ar in responses:
                    if ar.trigger.lower() in message.text.lower():
                        await message.reply(ar.response, parse_mode="HTML")
                        # Не делаем return, чтобы дальше сработала модерация (если нужно)
        except TelegramRetryAfter as e:
            print(f"⏳ Flood control в автоответчике. Ждем {e.retry_after} сек.")
            await asyncio.sleep(e.retry_after)
        except Exception as e:
    ...
    try:
        ...
        if user_status.status in ["left", "kicked"]:
            try:
                ...
                print(f"⏳ Flood control в проверке подписки. Ждем {e.retry_after} сек.")
                await asyncio.sleep(e.retry_after)
            except Exception as e:
                print(f"⚠️ Ошибка отправки предупреждения о подписке: {e}")
            return False
        return True
    except Exception as e:
