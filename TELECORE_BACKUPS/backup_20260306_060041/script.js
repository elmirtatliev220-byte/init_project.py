// Жёстко указываем адрес твоего сервера ngrok
const NGROK_URL = "https://felicidad-selfsame-veritably.ngrok-free.dev";
let API_URL = NGROK_URL;
let API_BASE_URL = NGROK_URL;

// Исправляем функцию, чтобы она стучалась по правильному адресу
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (data.api_url) {
                ...
                console.log("🔄 API_URL обновлен:", API_URL);
            }
        }
    } catch (e) { console.error("Ошибка конфига:", e); }
}
...
async function nextOnboardingStep() {
    ...
    if (currentOnboardingStep < onboardingSteps.length) {
        // Если это не последний шаг, просто показываем следующий слайд
        renderOnboardingStep();
        tg.HapticFeedback.impactOccurred('light');
    } else {
        // Последний шаг: выполняем логин и закрываем онбординг
        try {
            // Вызываем существующую функцию логина, которая получит токен
            await login();
            // После успешного логина можно сразу подгрузить каналы
            await initChannels();
        } catch (err) {
            console.error("Onboarding final login failed:", err);
            // Показываем ошибку пользователю, если логин не удался
            tg.showAlert("Не удалось подключиться к серверу. Попробуйте перезапустить приложение.");
        } finally {
            // В любом случае закрываем онбординг, чтобы не блокировать интерфейс
            closeWelcome();
        }
    }
}
...
async function login() {
    try {
        ...
        if (!res.ok) {
            const errorData = await res.json().catch(() => ({ detail: "Ошибка авторизации" }));
            throw new Error(errorData.detail || "Не удалось получить токен");
        }
        ...
        throw e; // Перебрасываем ошибку, чтобы вызывающий код (например, онбординг) мог ее обработать
    }
}
...
function openEditor() {
    console.log("DEBUG: openEditor clicked");
    // 1. Сначала показываем вкладку с редактором
    showTab('main');
        
    // 2. Информируем пользователя, если канал не выбран
    if (!currentChannelId || currentChannelId === 'null') {
        setTimeout(() => {
            tg.showAlert("Сначала выберите канал (кнопка '+' вверху), чтобы создать пост.");
        }, 100);
    }
}
...
// --- НОВАЯ ЛОГИКА ДЛЯ ЧИПОВ (2026) ---
function selectAIChip(element, action) {
    // Снимаем класс active со всех чипов
    document.querySelectorAll('#ai-action-chips .chip').forEach(chip => {
        chip.classList.remove('active');
    });
    // Добавляем класс active к нажатому
    element.classList.add('active');
    // Сохраняем выбранное действие в скрытое поле
    document.getElementById('ai-selected-action').value = action;
    tg.HapticFeedback.selectionChanged();
}

let currentAIStyle = "";

function setAIStyle(style, btn) {
...
function injectStyleChips() {
    const container = document.getElementById('ai-input-area');
    if (!container || document.getElementById('style-chips-container')) return;

    const chipsDiv = document.createElement('div');
    chipsDiv.id = 'style-chips-container';
    chipsDiv.className = "flex gap-2 mb-3 overflow-x-auto no-scrollbar";
    
    const styles = [
        { label: "🔥 Хайп", val: "Стиль: Эмоциональный, хайповый, виральный." },
        { label: "📚 Польза", val: "Стиль: Экспертный, полезный, обучающий." },
        { label: "📢 Новость", val: "Стиль: Новостной, сухой, факты." }
    ];

    styles.forEach(s => {
        const btn = document.createElement('button');
        btn.className = "style-chip px-3 py-1.5 rounded-full bg-gray-800 text-gray-400 text-[10px] font-bold whitespace-nowrap border border-gray-700 transition-all";
        btn.innerText = s.label;
        btn.onclick = () => setAIStyle(s.val, btn);
        chipsDiv.appendChild(btn);
    });

    container.insertBefore(chipsDiv, container.firstChild);
}
...
function toggleAI(show) {
    ...
    if (show) {
        ...
        // Сброс чипов на дефолт (clickbait)
        const defaultChip = document.querySelector('#ai-action-chips .chip');
        if(defaultChip) selectAIChip(defaultChip, 'clickbait');
        tg.BackButton.show();
    } else {
        modal.classList.add('hidden');
        tg.BackButton.hide();
...
async function submitAI() {
    ...
    const action = document.getElementById('ai-selected-action').value || 'clickbait';
    
    if (currentAIStyle) {
        prompt = `${currentAIStyle}\n\n${prompt}`;
    }

    // ИСПРАВЛЕНИЕ: Правильно закрытые условия
    ...
    try {
        const res = await fetch(`${API_BASE_URL}/api/ai/process`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
...
// --- ИНИЦИАЛИЗАЦИЯ ПРИЛОЖЕНИЯ (БЕЗ ЗАВИСАНИЙ) ---
(async () => {
    try {
        tg.ready();
        tg.expand();
        
        // 1. Сначала загружаем интерфейс и язык, чтобы кнопки ПОЯВИЛИСЬ сразу
        changeLanguage(currentLang);
        
        // 2. Загружаем конфиг, но не блокируем выполнение
        initConfig().catch(e => console.error("Ошибка конфига:", e));
        
        // 3. Пытаемся залогиниться и получить каналы в фоновом режиме
        // УБРАНО await: приложение не зависнет, если сервер тормозит
        login().then(() => {
            console.log("Авторизация успешна, обновляем список каналов...");
            return initChannels();
        }).catch(e => {
            console.error("Ошибка авторизации (возможно, сервер спит):", e);
        });
        ...
        console.log("✅ Скрипт запущен, интерфейс разблокирован!");

    } catch (criticalError) {
        console.error("Критическая ошибка запуска:", criticalError);
