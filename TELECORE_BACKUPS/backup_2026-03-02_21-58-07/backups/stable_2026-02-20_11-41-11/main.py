...
async def scheduler():
    ...
    last_metric_update = datetime.utcnow() - timedelta(hours=1, minutes=5)
    while True:
        try:
            # --- АВТОМАТИЧЕСКОЕ ОБНОВЛЕНИЕ ОХВАТОВ (ТЗ 4.2.2) ---
            # Запускаем раз в час
            if datetime.utcnow() - last_metric_update > timedelta(hours=1):
            ...
            async with async_session() as session:
                # ИСПРАВЛЕНО: Используем честный UTC
                now = datetime.utcnow()
                ...
                for post in posts:
                    try:
                        ...
                        post.is_sent = 1
                        print(f"✅ [{datetime.utcnow().strftime('%H:%M:%S')}] Пост {post.id} успешно отправлен в канал {post.channel_id}")
