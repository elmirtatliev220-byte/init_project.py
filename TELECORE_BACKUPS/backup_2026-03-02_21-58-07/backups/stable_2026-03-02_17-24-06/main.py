...
from app.core.models import Base, User, Channel, StatsHistory, PostStat, ScheduledPost, AdChannel, GroupProtection, TrafficSource, AutoResponse, AIRequest, WalletRequest, ChannelDeleteRequest, MovePostRequest, AutoDistributeRequest, ChatAnalytics, LoginRequest
...
async def toggle_protection(data: ProtectionRequest, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    # Используем ORM
    stmt = select(Channel).where(Channel.tg_id == data.chat_id)
    result = await db.execute(stmt)
    channel = result.scalar_one_or_none()
