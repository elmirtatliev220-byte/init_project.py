...
from app.core.models import Base, User, Channel, StatsHistory, PostStat, ScheduledPost, AdChannel, GroupProtection, TrafficSource, AutoResponse, AIRequest, WalletRequest, ChannelDeleteRequest, MovePostRequest, AutoDistributeRequest, ChatAnalytics, LoginRequest, ProtectionRequest, DeleteRequest, ChannelAddRequest, ChannelCheckRequest, ClearQueueRequest, AdRequest, InviteRequest, AutoResponseAddRequest
...
async def login_user(data: LoginRequest, db: AsyncSession = Depends(get_db)):
    ...
    # --- DEV-MODE BYPASS ---
    # Для локальной разработки, если initData невалидна, можно залогиниться как админ,
    # отправив в initData, например, "dev"
    if not tg_user and "dev" in data.initData:
        print("⚠️ ДЕВ-РЕЖИМ: Вход от имени администратора.")
        tg_user = {"id": settings.ADMIN_ID, "first_name": "Admin", "username": "admin"}
