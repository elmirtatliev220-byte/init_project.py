...
function promptAddChannel() {
    tg.showPopup({
        ...
        message: 'Введите ID канала (например, -100...). Бот должен быть администратором в этом канале.',
        buttons: [{id: 'ok', type: 'default', text: 'Ввести ID'}, {type: 'cancel'}]
    }, (btnId) => {
        if (btnId === 'ok') {
            const chanId = prompt("Введите ID канала:");
            if(!chanId) return;
            addChannelToServer(chanId);
        }
    });
}
...
async function addChannelToServer(chanId) {
    ...
    try {
        const res = await apiFetch(`/api/add_channel`, {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({ tg_id: parseInt(chanId) })
        });
