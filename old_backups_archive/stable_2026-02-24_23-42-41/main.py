...
# --- ТЕ САМЫЕ ИСПРАВЛЕНИЯ ДЛЯ УСТРАНЕНИЯ 404 ---

# 1. Подключаем статику (для картинок, если они нужны в HTML)
app.mount("/static", StaticFiles(directory="static"), name="static")

# 2. Главный эндпоинт, который открывает Mini App
@app.get("/")
async def serve_index():
    """Отдает главную страницу index.html"""
    if os.path.exists("index.html"):
        return FileResponse("index.html")
    return {"error": "Файл index.html не найден в корневой директории проекта"}

# --- ЗАПУСК ---

if __name__ == "__main__":
    # Используем настройки порта из твоего правила: 8080
