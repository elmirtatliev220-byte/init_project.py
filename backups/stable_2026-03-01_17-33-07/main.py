...
# --- ИНИЦИАЛИЗАЦИЯ APP (В самом верху) ---
app = FastAPI()
...
# Подключаем lifespan к уже созданному app
...
async def resolve_channel(data: ChannelCheckRequest, user: User = Depends(get_current_user)):
    """Проверяет канал по ссылке перед добавлением, используя общую логику из бота."""
    try:
