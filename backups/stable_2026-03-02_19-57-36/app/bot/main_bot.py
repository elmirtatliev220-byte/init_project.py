...
# Импорты для работы с БД, чтобы избежать циклической зависимости с main.py
from app.core.models import *
from sqlalchemy import select, delete, update
from app.core.database import async_session
from app.core.protection import analyze_protection_logic, check_spam
# Инициализация
...
async def on_user_join(event: types.ChatMemberUpdated):
    ...
    try:
        async with async_session() as session:
            ...
            # Логируем вход для аналитики
            session.add(ChatAnalytics(chat_id=event.chat.id, event_type='join'))
            await session.commit()
...
async def moderator_handler(message: types.Message):
    ...
    try:
        ...
        pass # Если не можем проверить, считаем, что не админ

    # Логируем сообщение для аналитики
    try:
        async with async_session() as session:
            session.add(ChatAnalytics(chat_id=message.chat.id, event_type='message'))
            await session.commit()
    except Exception:
        pass

    if not is_admin:
        # 1. ПРОВЕРКА ПОДПИСКИ (только для не-админов)
        # ... (логика проверки подписки остается без изменений)

        # 2. НОВАЯ ЛОГИКА ЗАЩИТЫ (только для не-админов)
        text = message.text or message.caption or ""
        if text:
            result = await check_spam(text, message.from_user.id, message.chat.id)
            if result["action"] == "delete":
                await message.delete()
                # Логируем спам
                try:
                    async with async_session() as session:
                        session.add(ChatAnalytics(chat_id=message.chat.id, event_type='spam'))
                        await session.commit()
                except: pass
                return
    ...
    # 3. АВТООТВЕТЧИК (срабатывает для всех, если сообщение не было удалено)
    if message.text:
        try:
            async with async_session() as session:
...
# --- ФУНКЦИЯ ПОИСКА КАНАЛА (ДЛЯ API) ---
async def resolve_channel_by_link(link: str, user_id: int):
    """
    Находит канал по ссылке, проверяет права бота и пользователя.
    ...
    try:
        ...
        raise ValueError(f"Не удалось найти канал: {identifier}")

    if chat.type not in ["channel", "supergroup"]:
        raise ValueError("Это не канал и не супергруппа.")
    ...
    # Проверка прав пользователя (мы не проверяем, так как API вызывает владелец, 
    # но для надежности можно проверить, является ли юзер админом в чате)
    # В данном ТЗ мы доверяем тому, что если юзер добавил бота, он имеет права, 
    # но resolve_channel_by_link может использоваться и для проверки.
