...
async def check_spam(text: str, user_id: int, chat_id: int):
    """
    Базовая логика проверки спама. 
    Использует стоп-слова и проверку ссылок из конфигурации.
    """
    # Проверка стоп-слов
    for word in STOP_WORDS:
        if word.lower() in text.lower():
            return {"action": "delete", "reason": "stop_word"}
    
    # Проверка ссылок
    if not ALLOW_LINKS:
        if "http" in text or "t.me" in text:
            return {"action": "delete", "reason": "link_prohibited"}
            
    return {"action": "none"}

async def analyze_protection_logic(session, channel_id: int):
    """
    Анализирует статус защиты канала (права бота).
    ...
    try:
        ...
        return {
            "status": "success" if (is_admin and can_delete) else "warning",
            "bot_is_admin": is_admin,
            "can_delete": can_delete,
            "can_restrict": can_restrict,
            "message": "Защита активна" if is_admin else "Требуются права администратора"
        }
    except Exception as e:
